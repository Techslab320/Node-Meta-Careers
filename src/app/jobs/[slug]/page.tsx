  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8">
        <Link href="/jobs" className="inline-flex items-center gap-2 text-sm text-cyan-300 hover:text-cyan-200">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {POSITION_EMPLOYMENT_TYPES.map((type) => (
            <Badge key={type} className="border-cyan-500/20 bg-cyan-500/10 text-cyan-100">
              {formatLabel(type)}
            </Badge>
          ))}
          {POSITION_EXPERIENCE_LEVELS.map((level) => (
            <Badge key={level} className="border-emerald-500/20 bg-emerald-500/10 text-emerald-100">
              {formatLabel(level)}
            </Badge>
          ))}
        </div>
                ...
                <tbody>
                  {job.compensationBands.map((band) => (
                    <tr key={`${band.employmentType}-${band.experienceLevel}`} className="border-b border-slate-900/80">
                      ...
                      <td className="py-2 text-cyan-300">{formatCompensationBand(band)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
