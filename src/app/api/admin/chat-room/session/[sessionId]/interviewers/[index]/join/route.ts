...
import { getAdminChatRoomSessionById, joinHrInterviewer } from "@/lib/chat-room/sessions";
...
export async function PATCH(_request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const session = await joinHrInterviewer(sessionId, interviewerIndex);
    if (!session) {
      return NextResponse.json({ error: "Session not found" }, { status: 404 });
    }
    ...
    console.error("Failed to join HR interviewer", error);
    return NextResponse.json({ error: "Unable to join interviewer" }, { status: 500 });
  }
}
