...
export async function PATCH(_request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    if (!existing.joinedHrInterviewerIndexes.includes(interviewerIndex)) {
      return NextResponse.json({ error: "Interviewer is not in the room" }, { status: 400 });
    }

    const session = await leaveHrInterviewer(sessionId, interviewerIndex);
    if (!session) {
      return NextResponse.json({ error: "Session not found" }, { status: 404 });
    }
    ...
    console.error("Failed to leave HR interviewer", error);
    return NextResponse.json({ error: "Unable to leave interviewer" }, { status: 500 });
  }
}
